...
def find_files(data_dir: Path(), fname_glob: str, verbose: bool = True):
    path_iter = data_dir.rglob(fname_glob)
    if verbose:
        path_iter = tuple(path_iter)
        print(f'{len(path_iter)} paths matching {fname_glob} found in {data_dir}:')
        print_iter(
...
def run_pool(process_to_run, inputs, log_level: int = 30):
    ...
    with mp.Pool(processes=cpus) as pool:
        ...
        # BUG: issue with `source/analyze_deps.py -g \*\[\!j\]/\*exactly\*hits.pkl.gz -n NEG-exactly -s 0 -t`
...
def print_iter(iter_obj, bullet: str = '▸'):
    print(f'{bullet} '+f'\n{bullet} '.join(str(i) for i in iter_obj))


# HACK: for testing
if __name__ == '__main__':
    run_pool(process_to_run=max,
             inputs=([x]*(x+1)+[1] for x in range(6)),
             log_level=20)
