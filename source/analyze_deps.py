...
def _main():
    ...
    for cross_col in crosses:
        ...
        print(
            # f'\n   total time elapsed: `{ugen.dur_round((_t1 - _t0).seconds)}`')
...
def _get_dep_args(args):
    ...
    # if args.verbose:
    #     print_paths = paths_df.copy()
    #     print_paths.loc[:, 'dataset'] = [Path(p).name.split('.', 1)[0] for p in print_paths.input]
...
def _get_crosstab_label(name, df_sample_label=None):
    ct_out_label = (name or pd.Timestamp.now().strftime("%Y-%m-%d_%H:%M"))
    sample_tag = df_sample_label or ''
