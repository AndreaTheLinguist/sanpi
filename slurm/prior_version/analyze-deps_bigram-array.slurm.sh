...
DATE="$(date +%y-%m-%m_%H%M)"
LOG_FILE=${LOGS_DIR}/${SLURM_JOB_NAME}-${SEED}_${DATE}.log
echo "Combined log will be appended to ${LOG_FILE}"

echo -e "time python ${SOURCE_DIR}/analyze_deps.py -g ${SEED}*hits.pkl.gz -n bigram-${SEED} -v >> >(tee -i -a ${LOG_FILE}) 2>&1"
time python ${SOURCE_DIR}/analyze_deps.py -g ${SEED}*hits.pkl.gz -n bigram-${SEED} -v >> >(tee -i -a ${LOG_FILE}) 2>&1
