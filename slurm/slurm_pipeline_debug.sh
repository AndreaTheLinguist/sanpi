...
# > single specific subdirectory
BUG_CORP="${DEBUG_DIR}/bigram_debug/bigram-apw"

echo "time python ${PROG} -c ${BUG_CORP} -p ${PAT_ARG} -g ${DEBUG_DIR}/1_json_grew-matches"
time python ${PROG} -c ${BUG_CORP} -p ${PAT_ARG} -g ${DEBUG_DIR}/1_json_grew-matches

# > all subdirectories
# for BUG_CORP in ${DEBUG_DIR}/bigram_debug/bigram*; do
