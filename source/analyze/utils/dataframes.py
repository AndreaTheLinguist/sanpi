...
def balance_sample(full_df: pd.DataFrame(),
                   ...
                   verbose: bool = False):
    '''
    create sample with no more than n rows satisfying each unique value of the given column. 
    A value of 0 for `sample_per_value` will limit all values' results to the minimum count per value.
    '''
    sub_samples = []
    n = 5 if not sample_per_value else sample_per_value

    for c in full_df.loc[:, column_name].unique():
        sdf = full_df.loc[full_df[column_name] == c, :].sample(
            min(full_df.value_counts(subset=column_name)[c], n))
    ...
    if verbose:
        subset_info = b_sample.value_counts(subset=column_name).to_frame(name='count').assign(
            percentage=b_sample.value_counts(column_name, normalize=True).round(2) * 100).to_markdown()
...
def concat_pkls(data_dir: Path() = Path('/share/compling/data/sanpi/2_hit_tables'),
    ...
    pickles = data_dir.rglob(fname_glob)
    if verbose:
        pickles = tuple(pickles)
        print(f'{len(pickles)} paths matching {fname_glob} found in {data_dir}:')
        print_iter([f'../{p.relative_to(data_dir)}' for p in pickles])
    ...
    df = df.loc[~df.duplicated(cols_by_str(
        df, end_str=('text', 'id', 'sent'))), :]
    df = make_cats(df, ['corpus']+cols_by_str(df,
                   start_str=('nr', 'neg', 'adv'), end_str=('lemma', 'form')))
