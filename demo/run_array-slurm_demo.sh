...
# /home/arh234/projects/sanpi/demo/run_array-slurm_demo.sh
...
if [[ `which grew && grew version` ]]; then
    ...
    for pat in "${pats[@]}"; do 
        echo -e "\n# ${pat}"
        echo "sbatch -J ${pat}DEMO exactly-array.demo-slurm.sh ${pat}"
        sbatch -J ${pat}DEMO exactly-array.demo-slurm.sh ${pat};
    done
else
